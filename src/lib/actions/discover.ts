...
export async function getDiscoverBuildings(filters?: {
    ...
    try {
        ...
        if (filters?.flatType) {
            filtered = filtered.filter(b => b.flatTypes.includes(filters.flatType as any))
        }

        if (filters?.minBudget && filters.minBudget > 0) {
            filtered = filtered.filter(b => b.rentRange && b.rentRange.max >= filters.minBudget!)
        }

        if (filters?.maxBudget && filters.maxBudget > 0) {
            filtered = filtered.filter(b => b.rentRange && b.rentRange.min <= filters.maxBudget!)
        }

        return { success: true, data: filtered }
    } catch (error: any) {
        console.error("Failed to fetch discover buildings:", error)
        return { error: `Failed to load buildings: ${error.message || String(error)}` }
    }
}
...
export async function getDiscoverBuildingDetail(buildingId: string) {
    try {
        ...
        const allRents = building.flats.map(f => f.rentAmount).filter(r => r > 0)
        const flatTypes = [...new Set(building.flats.map(f => f.flatType))]

        // Group vacant flats by type for display
        const vacantByType: Record<string, { count: number; rentRange: { min: number; max: number } }> = {}
        for (const flat of vacantFlats) {
            ...
            vacantByType[type].rentRange.min = Math.min(vacantByType[type].rentRange.min, flat.rentAmount)
            vacantByType[type].rentRange.max = Math.max(vacantByType[type].rentRange.max, flat.rentAmount)
        }
        ...
        return { error: `Failed to load building: ${error.message || String(error)}` }
    }
}
...
export type DiscoverInquiryInput = z.infer<typeof discoverInquirySchema>
...
export async function submitDiscoverInquiry(data: DiscoverInquiryInput) {
    ...
    const { buildingId, name, phone, email, preferredBHK, budget, message, source, utmSource, clausesAccepted } = result.data
    ...
    try {
        ...
        return { error: `Failed to submit inquiry: ${error.message || String(error)}` }
    }
}
...
export async function logQRScan(buildingId: string, userAgent?: string, referrer?: string) {
    try {
